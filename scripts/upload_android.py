...
def uploadToPgyer(AppPath,updateDescription):
    print "Begin to upload apk to Pgyer: %s" % AppPath
    headers = {'enctype': 'multipart/form-data'}
    postDate = {
        'uKey': USER_KEY,
        '_api_key': API_KEY,
        'buildInstallType': '2', #(必填)应用安装方式，值为(2,3，4)。2：密码安装，3：邀请安装，4：回答问题安装。 默认为 2密码安装
...
if __name__ == '__main__':
    ...
    if sys.argv[1]=="Student":
        appname = os.path.join(workspace,"Rxt_Android/phone/app/build/outputs/apk/debug/app-debug.apk")
        QRCodePath =os.path.join(workspace,"Rxt_Android/phone/app/build/outputs/apk/debug/QRCode.jpg")
    elif sys.argv[1]=="Teacher":
        appname= os.path.join(workspace,"ShangXueYuan_Android/app/build/outputs/apk/debug/app-debug.apk")
        QRCodePath =os.path.join(workspace,"ShangXueYuan_Android/app/build/outputs/apk/debug/QRCode.jpg")
    QRCodeUrl=uploadToPgyer(appname, sys.argv[1])
    print "QRCodeUrl is :",QRCodeUrl
